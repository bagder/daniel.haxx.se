 ...
 *   a 4 bytes header (instead of previous 8) just as FRAGMENTS.
 *
 * v2.2
...
/* This is the data kept in all _free_ FRAGMENTS. This struct maps to the
   same address as struct MemInfo. */
struct MemFrag {
  void *block;            /* same as in MemInfo, but this is set to NULL
                             to detect double free attempts */
...
static void FragBlock(char *memp, int size)
{
  ...
  while((count+size) <= DMEM_BLOCKSIZE) {
    frag->block = NULL; /* Mark the FRAGMENT as free */
...
void dfree(void *memp)
{
  ...
  /* Try to detect if a fragment has been freed twice. This check
     may still fail if the parent BLOCK has been bfree()'d */
  if(meminfo->block == NULL) {
      DBG(("ERROR! Memory block %p freed twice!\n", memp));
      return;
  }
  ...
  if(!((size_t)meminfo->block&BLOCK_BIT)) {
    ...
    if(block->nfree == memtop->nmax) {
      ...
      /* Place this block first in the list, so malloc() won't have
         to search the list for blocks with free frags */
